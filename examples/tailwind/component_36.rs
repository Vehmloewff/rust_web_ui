...
impl Widget<'_> for Example36 {
	...
	fn render(mut ctx: Ctx<'_>, _props: Example36Props) {
		ctx.styles(vec![Style::Noop("")]);
		ctx.set_attribute("style", "");
		
		ctx.child("1", Dynamic).run("div", |mut props| {
			props.styles(vec![Style::Width(Size::Exact(0)), Style::Noop("min-h-[768px]"), Style::Noop("bg-white")]);
			...
			props.child("3", Dynamic).run("main", |mut props| {
				props.styles(vec![Style::Noop("grid"), Style::Noop("min-h-full"), Style::Noop("place-items-center"), Style::Noop("bg-white"), Style::PaddingX(Size::Exact(24)), Style::PaddingY(Size::Exact(96)), Style::OnScreen(Screen::Small, &[Style::PaddingY(Size::Exact(128))]), Style::OnScreen(Screen::Large, &[Style::PaddingX(Size::Exact(32))])]);
				
				props.child("1", Dynamic).run("div", |mut props| {
					props.styles(vec![Style::Noop("text-center")]);
					
					props.child("1", Dynamic).run("p", |mut props| {
						props.styles(vec![Style::Noop("text-base"), Style::FontSemibold, Style::Noop("text-indigo-600")]);
						
						props.child("0", Label).run(|props| props.text("404"));
					});
					props.child("3", Dynamic).run("h1", |mut props| {
						props.styles(vec![Style::MarginTop(Size::Exact(16)), Style::Noop("text-3xl"), Style::FontBold, Style::Noop("tracking-tight"), Style::TextColor(Color::Fg(100)), Style::OnScreen(Screen::Small, &[Style::Noop("text-5xl")])]);
