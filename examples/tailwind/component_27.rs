...
impl Widget<'_> for Example27 {
	...
	fn render(mut ctx: Ctx<'_>, _props: Example27Props) {
		...
		ctx.child("1", Dynamic).run("div", |mut props| {
			...
			props.child("1", Dynamic).run("div", |mut props| {
				...
				props.child("1", Dynamic).run("dl", |mut props| {
					props.styles(vec![Style::Noop("grid"), Style::Noop("grid-cols-1"), Style::Noop("gap-x-8"), Style::Noop("gap-y-16"), Style::Noop("text-center"), Style::OnScreen(Screen::Large, &[Style::Noop("grid-cols-3")])]);
					
					props.child("1", Dynamic).run("div", |mut props| {
						props.styles(vec![Style::Noop("mx-auto"), Style::Flex, Style::Noop("max-w-xs"), Style::Noop("flex-col"), Style::Noop("gap-y-4")]);
						
						props.child("1", Dynamic).run("dt", |mut props| {
							props.styles(vec![Style::Noop("text-base"), Style::Noop("leading-7"), Style::TextColor(Color::Fg(67))]);
							
							props.child("0", Label).run(|props| props.text("Transactions every 24 hours"));
						});
						props.child("3", Dynamic).run("dd", |mut props| {
							props.styles(vec![Style::Noop("order-first"), Style::Noop("text-3xl"), Style::FontSemibold, Style::Noop("tracking-tight"), Style::TextColor(Color::Fg(100)), Style::OnScreen(Screen::Small, &[Style::Noop("text-5xl")])]);
